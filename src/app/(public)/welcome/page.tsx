...
export default function WelcomePage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    // On success, user is redirected to Google — no need to clear loading
  };
  ...
  const handleAppleSignIn = async () => {
    ...
    if (error) {
      setMessage(error.message);
      setMessageType("error");
      trackLoginError("apple", error.message);
      setLoading(null);
    }
  };
      ...
      <div className="w-full max-w-[320px] animate-fade-in-up animation-delay-300">
        ...
        <div className="space-y-2.5 mb-4">
          ...
          <button
            ...
            disabled={loading !== null}
            className="flex w-full items-center justify-center gap-3 rounded-2xl bg-ink px-4 py-3.5 text-[14px] font-medium text-porcelain active:scale-[0.98] transition-all duration-200 disabled:opacity-50"
          >
